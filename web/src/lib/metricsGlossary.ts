...
const M: MetricInfo[] = [
  ...
  {
    key: "annualized-return", title: "Annualized Return", category: "Risk-Adjusted Performance",
    description: "The portfolio's daily returns compounded and scaled to a yearly rate.",
    why: "It's the numerator of every risk-adjusted ratio — the reward side of the reward-for-risk question.",
    formula: "AnnRet = (Π(1 + rₜ))^(252 / n) − 1",
    benchmark: "Judge relative to the benchmark and the risk taken, not in isolation.",
    mistakes: "Reading an annualized figure off only a few weeks of data as if it were a stable long-run rate.",
    related: ["sharpe", "volatility", "jensens-alpha"],
  },
  {
    key: "treynor", title: "Treynor Ratio", category: "Risk-Adjusted Performance",
    description: "Excess return earned per unit of systematic (market) risk, i.e. per unit of beta.",
    why: "For a diversified book, only market risk should be compensated. Treynor isolates that, where Sharpe uses total risk.",
    formula: "Treynor = (Return − Risk-Free) / Portfolio Beta",
    interpretation: "Higher is better. Compare to the benchmark's own (Return − rf) — a portfolio above it is paying off per unit of market risk.",
    benchmark: "Most meaningful for well-diversified portfolios; unstable when beta is near zero.",
    related: ["sharpe", "portfolio-beta", "jensens-alpha"],
  },
  {
    key: "jensens-alpha", title: "Jensen's Alpha", category: "Risk-Adjusted Performance",
    description: "Return above (or below) what CAPM predicts for the portfolio's market exposure.",
    why: "It's the cleanest single read on skill: value added beyond simply taking market risk.",
    formula: "α = Rₚ − [ Rf + β · (R_bench − Rf) ]",
    bands: [
      { range: "> 0", label: "Outperformed expected", tone: "good" },
      { range: "≈ 0", label: "In line with market", tone: "neutral" },
      { range: "< 0", label: "Underperformed expected", tone: "bad" },
    ],
    benchmark: "Persistent positive alpha is rare and valuable; a few weeks of positive alpha is not yet skill.",
    related: ["treynor", "portfolio-beta", "information-ratio"],
  },
  {
    key: "portfolio-beta", title: "Portfolio Beta", category: "Risk-Adjusted Performance",
    description: "Sensitivity of the portfolio's returns to the chosen benchmark's returns.",
    why: "Tells you how much of your risk is simply market exposure, and how the book behaves in a broad rally or selloff.",
    formula: "β = Cov(Rₚ, R_bench) / Var(R_bench)",
    bands: [
      { range: "< 1", label: "Defensive", tone: "good" },
      { range: "≈ 1", label: "Market-neutral", tone: "neutral" },
      { range: "> 1", label: "Aggressive", tone: "warn" },
    ],
    benchmark: "Choose the benchmark deliberately — beta vs the S&P differs from beta vs the Nasdaq 100.",
    related: ["treynor", "jensens-alpha", "tracking-error"],
  },
  {
    key: "information-ratio", title: "Information Ratio", category: "Risk-Adjusted Performance",
    description: "Active return (portfolio minus benchmark) per unit of tracking error.",
    why: "The standard yardstick for active management skill: how much excess return you earn for each unit of benchmark-relative risk.",
    formula: "IR = (Return − Benchmark Return) / Tracking Error",
    bands: [
      { range: "> 0.75", label: "Strong skill", tone: "good" },
      { range: "0.4–0.75", label: "Good", tone: "good" },
      { range: "0–0.4", label: "Modest", tone: "warn" },
      { range: "< 0", label: "Lagging benchmark", tone: "bad" },
    ],
    benchmark: "Top active managers sustain ~0.5+ over full cycles.",
    related: ["tracking-error", "jensens-alpha"],
  },
  {
    key: "tracking-error", title: "Tracking Error", category: "Risk-Adjusted Performance",
    description: "Annualized volatility of the portfolio's return difference versus the benchmark.",
    why: "Measures how far the book strays from the benchmark — the 'active risk' you take to try to outperform.",
    formula: "TE = StdDev(Rₚ − R_bench) × √252",
    interpretation: "Higher tracking error = more active, more benchmark-relative deviation. Low TE ≈ near index-hugging.",
    benchmark: "Index funds run ~0–2%; active equity mandates often 3–8%.",
    related: ["information-ratio", "portfolio-beta"],
  },
  {
    key: "downside-deviation", title: "Downside Deviation", category: "Risk-Adjusted Performance",
    description: "Volatility computed only from returns below a minimum acceptable return (default 0%).",
    why: "Investors fear losses, not upside. Downside deviation is a more investor-relevant risk measure than total volatility.",
    formula: "DD = √( mean( min(0, rₜ − MAR)² ) ) × √252",
    interpretation: "Only harmful (below-target) moves contribute. If it's well below total volatility, most variability is on the upside.",
    benchmark: "It is the denominator of the Sortino ratio.",
    related: ["sortino", "volatility", "cvar"],
  },
  {
    key: "risk-efficiency-score", title: "Risk-Efficiency Score", category: "Risk-Adjusted Performance",
    description: "A 0–100 blend of Sharpe, Sortino, Jensen's alpha, beta health, max drawdown, downside deviation and information ratio.",
    why: "One glanceable read on whether the portfolio is being adequately compensated for the risk it takes.",
    formula: "0.22·Sharpe + 0.18·Sortino + 0.15·Alpha\n+ 0.13·MaxDD + 0.12·Beta + 0.10·Downside + 0.10·IR\n(each mapped to 0–100)",
    bands: [
      { range: "85–100", label: "Excellent", tone: "good" },
      { range: "70–84", label: "Strong", tone: "good" },
      { range: "55–69", label: "Moderate", tone: "warn" },
      { range: "< 55", label: "Weak", tone: "bad" },
    ],
    benchmark: "A house score, not an industry standard — use it for tracking and comparison over time.",
    related: ["sharpe", "sortino", "jensens-alpha", "information-ratio"],
  },
];
